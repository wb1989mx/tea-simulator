...
## 工艺模拟 v3（环境 + 时间双控制 + 黑茶倒U）

- **黑茶采摘·倒U型判定**：黑茶按黑毛茶嫩度等级评定——单芽/过嫩原料在渥堆中易粘结成团、发酵不匀，制作效果并不理想；**一芽二三叶（高档黑毛茶基准）为最优**，过嫩与过粗老均降分。与乌龙茶「成熟度适当」不同，黑茶曲线峰点在一芽二三叶，单芽（35分）低于一二叶（80分）。
- **温度尺度防护**：全部工艺温度滑块的 min/max 已逐一核对，保证最适温度区间始终落在可调范围内；套用名优茶参数、嫩度偏移、环境调整后，运行时自动扩展滑块边界，杜绝「理想值滑不到」的情况。
- **时间控制条**：所有涉及时间控制的工序均增加时间滑块——杀青、干燥、茶坯复火、烘焙、渥堆等温度类工序现在是「温度 + 时间」双条控制，评分按温度60%+时间40%加权；揉捻、闷黄、萎凋、发酵、窨花、通花等原有时间工序保持不变。时间理想值随采摘嫩度与环境自动调整。
- **环境前提背景**：制茶开始前需设定当日环境（晴天/多云/阴天/雨天/低温/高温干燥，含温度与湿度）。环境直接影响各工序合理参数：如雨天严禁晒青、萎凋需延长50%且必须加温；低温发酵需保温并延长35%；高温干燥需遮阴缩短萎凋。每道工序顶部实时显示该环境下的工艺调整提示，并按环境适配度轻微计入工序得分。
